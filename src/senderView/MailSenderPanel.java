...
public class MailSenderPanel extends JPanel implements Runnable, GetResult, MouseListener {
	...
	public MailSenderPanel() {
		...
		{/*「宛先」欄、「CC」欄共同のパネルを生成 */
			...
			//宛先欄とCC欄
			JLabel lblTo = new JLabel("<html>　宛先　　<br><br>　CC　　");
			lblTo.setFont(new Font("MS UI Gothic", Font.BOLD, 15));
		...
		this.add(attachment, "cell 14 5, span 1, grow");

		//余白
		this.add(new JLabel(""), "cell 15 5, span 1, grow");
		...
		txtDetail.setFont(new Font("MS UI Gothic", Font.BOLD | Font.ITALIC, 12));
			...
			setAllEditable(false);
			{
				...
				try {
					...
					if(attachFileList.size() > 0){
						/* 添付ファイルがある場合 */
						sender.sendMail(ccArray, txtSubject.getText(), txtDetail.getText(), attachFileList);
					} else {
						/* 添付ファイルがない場合 */
						sender.sendMail(ccArray, txtSubject.getText(), txtDetail.getText());
					}
	...
	public void setResult(Object retValue, int callNumber) {
		...
		case 0x1:
			...
			//CC欄に受け取ったアドレスを追加
			for(int i=0; i<toCC.length; i++, toCount++){
				if(toCount > 99){
					//送信先が99を超えたら例外を出す
				...
				try{
					...
					ccList.get(toCount).setText(toCC[i]);	//例外発生の可能性
					...
					JTextArea txtCC = new JTextArea(toCC[i]);
					txtCC.setBorder(new EtchedBorder(EtchedBorder.LOWERED));
		...
		case 0x2:
			...
			//CC欄に受け取ったアドレスを追加
			for(int i=0; i<bcc.length; i++, toCount++){
				if(toCount > 99){
					//送信先が99を超えたら例外を出す
					JOptionPane.showMessageDialog(null, "宛先が99件を超えています", "エラー", JOptionPane.OK_OPTION);
					throw new AddressLimitOverException("Send to Address is Over 99");
				...
				try{
					//既にある「BCC」欄にアドレスを追加
					bccList.get(toCount).setText(bcc[i]);	//例外発生の可能性
					...
					//未元なCC欄を追加、その欄にアドレスを追加
					JTextArea txtBCC = new JTextArea(bcc[i]);
					txtBCC.setBorder(new EtchedBorder(EtchedBorder.LOWERED));
